...
public class leetcode1319 {
    ...
    public int makeConnected(int n, int[][] connections) {
        ...
        for (int i = 0; i < a.length; i++) {
            int x = unionFind.find(a[i]);
            map.put(unionFind.find(a[i]),map.getOrDefault(x,0)+1);
        }
        if (some < map.size() - 1) {
            return -1;
        }else {
